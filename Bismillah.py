...
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] [\x1b[1;96m+%s]Author   : Baz Hengker Tzy
	...
	def __login__(self):
		...
		if token in ['']:
			...
			try:
				...
				jalan(' [%s!%s] Token error ngab coba ganti akun tumbal!'%(M,N))
				self.takon()
	def takon(self):
		takon = input('\n %s[%s!%s] Mau tau cara ambil token y/t: '%(N,O,N))
		if takon in ['y','Y','iya']:
			jalan('\n %s[%s!%s] Kamu akan di arahkan ke fb untuk meminta tutor ambil token'%(N,O,N))
...
class menu:
	...
	def main(self):
		...
		print(' %s[%s4%s] CRACK DARI LIKE POST'%(N,O,N))
		print(' %s[%s5%s] CRACK DARI FOLLOWERS'%(N,O,N))
		print(' %s[%s6%s] CEK OPSI CHECKPOINT'%(N,O,N))
